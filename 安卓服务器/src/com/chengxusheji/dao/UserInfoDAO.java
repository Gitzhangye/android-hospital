...
public class UserInfoDAO {
    ...
    public ArrayList<UserInfo> QueryUserInfoInfo(String user_name,String name,String birthday,String jiguan,int currentPage) { 
    	Session s = factory.getCurrentSession();
    	...
    	if(!jiguan.equals("")) hql = hql + " and userInfo.jiguan like '%" + jiguan + "%'";
    ...
    @Transactional(propagation=Propagation.NOT_SUPPORTED)
    public ArrayList<UserInfo> QueryUserInfoInfo(String user_name,String name,String birthday,String jiguan) { 
    	Session s = factory.getCurrentSession();
    	String hql = "From UserInfo userInfo where 1=1";
    	if(!user_name.equals("")) hql = hql + " and userInfo.user_name like '%" + user_name + "%'";
    	if(!name.equals("")) hql = hql + " and userInfo.name like '%" + name + "%'";
    ...
    /*计算总的页数和记录数*/
    @Transactional(propagation=Propagation.NOT_SUPPORTED)
    public void CalculateTotalPageAndRecordNumber(String user_name,String name,String birthday,String jiguan) {
        Session s = factory.getCurrentSession();
        String hql = "From UserInfo userInfo where 1=1";
        if(!user_name.equals("")) hql = hql + " and userInfo.user_name like '%" + user_name + "%'";
        if(!name.equals("")) hql = hql + " and userInfo.name like '%" + name + "%'";
