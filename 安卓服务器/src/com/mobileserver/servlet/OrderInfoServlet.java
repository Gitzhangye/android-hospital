...
public class OrderInfoServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		if (action.equals("query")) {
			...
			if(orderInfoDAO.QueryOrderInfo(uesrObj, doctor, orderDate, timeSlotObj, 0).size()>0) {
				response.setCharacterEncoding("utf-8");
				PrintWriter out = response.getWriter();
				out.print("该时间段你已经预约过这个医生了！");
				return;
			}
			
			DoctorDAO doctorDAO = new DoctorDAO();
			Doctor doctorObj = doctorDAO.GetDoctor(doctor);
			
			if(orderInfoDAO.QueryOrderInfo("", doctor, orderDate, 0, 0).size() >= doctorObj.getVisiteTimes()) {
				response.setCharacterEncoding("utf-8");
				PrintWriter out = response.getWriter();
				out.print("当天该医生就诊人次已满!");
				return;
			}
